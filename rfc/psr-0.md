...
- **Why this is a problem**: This section explains the effect of this problem might be, and why this scenario is such a bad thing.
- **Minimum Solution**: This section gives a solution to the problem that would take the least possible work on the part of the developer—but might provide little improvement on the users experience. This solution is probably already be in use by dependents.
- **WWJD Solution**: This section gives the ideal, or the "God solution". We basically ask the question, "What Would Jesus Do?" If we were all-powerful and had no constraints, how might we solve this problem? Asking the question in this way helps us to define "ideal" absolutely, rather than relative to what we mgiht normally term acheiveable.
...
We want to be transparent about this, because the user is installing something that is visible. That is, the dependency has forward-facing features, or, at the very least, will be visible in the list of installed plugins. If the user sees these features (or the plugin row) and doesn't know where they have come from, this will cause confusion.
...
#### Proposed Solution
The fact that the library is a dependency should be displayed to the user, and they shouldn't have the option to deactivate it alone. They should have to choose to deactivate both the plugin depenency and all of its dependents.
...
- First, it would be nice to have a way to check if there were any known incompatibilities before the update is even performed. This could involve an API endpoint on the library's distributor. We could also check the local dependencies definition (assuming one exists). 
...
Notes:
- Running dry-run updates with automated tests actually looks like the future to me. This is as close as you can humanly get to nothing-breaks updates. While it might take a lot of work to implement at first, I think it would be really worth it. We can start with simple dry-runs and work on the testing part as we go along. Related: WordPoints/wordpointsorg#35.
...
#### Proposed Solution
When the user updates the dependent, give them the option to update the dependency as well. This would require that we already know about the incompatibility via an API endpoint provided by the library disributor, or another means. Dry-run checks as noted in problem 3 could still be useful.
...
#### Minimum Solution
Dependents show a message telling the user that the dependency is the wrong version. (This will probably just confuse a frustrate the user in many cases.)
